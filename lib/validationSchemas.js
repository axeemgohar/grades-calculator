...
export const swissCourseSchema = z.object({
  id: z.number(),
  name: z.string().optional(),
  // Swiss scale: 1.0 to 6.0
  grade: z
    .string()
    .min(1, 'Bitte Note eingeben')
    .refine((val) => {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 1.0 && num <= 6.0;
    }, 'Note muss zwischen 1.0 und 6.0 liegen'),
  weight: z.string().refine((val) => {
    if (val === '') return true;
    const num = parseFloat(val);
    return !isNaN(num) && num > 0;
  }, 'Gewichtung muss positiv sein'),
});

export const swissCourseListSchema = z
  .array(swissCourseSchema)
  .min(1, 'Mindestens ein Fach erforderlich');

// Swiss Points Calculator Schema
export const swissPointsSchema = z.object({
  maxPoints: z
    .string()
    .min(1, 'Max. Punkte erforderlich')
    .refine((val) => parseFloat(val) > 0, 'Muss größer als 0 sein'),
  achievedPoints: z
    .string()
    .min(1, 'Erreichte Punkte erforderlich')
    .refine((val) => parseFloat(val) >= 0, 'Darf nicht negativ sein'),
});
...
// German Validation
export const germanCourseSchema = z.object({
  id: z.number(),
  name: z.string().optional(),
  // German grades are usually 1.0 to 6.0
  grade: z
    .string()
    .min(1, 'Bitte Note eingeben') // "Please enter grade"
    .refine((val) => {
      const num = parseFloat(val);
      return !isNaN(num) && num >= 0.7 && num <= 6.0;
    }, 'Note muss zwischen 1.0 und 6.0 liegen'),
  weight: z.string().refine((val) => {
    if (val === '') return true; // allow empty (defaults to 1 in logic)
    const num = parseFloat(val);
    return !isNaN(num) && num > 0;
  }, 'Gewichtung muss positiv sein'), // "Weight must be positive"
});

export const germanCourseListSchema = z
  .array(germanCourseSchema)
  .min(1, 'Mindestens ein Fach ist erforderlich'); // "At least one subject required"

// Netherlands Schema
export const netherlandsCourseSchema = z.object({
  id: z.number(),
  // Dutch scale: 1.0 to 10.0
  grade: z
    .string()
    .min(1, 'Vul een cijfer in') // "Enter a grade"
    .refine((val) => {
      const num = parseFloat(val.replace(',', '.'));
      return !isNaN(num) && num >= 1.0 && num <= 10.0;
    }, 'Cijfer moet tussen 1,0 en 10,0 liggen'),
  weight: z.string().refine((val) => {
    if (val === '') return true;
    const num = parseFloat(val.replace(',', '.'));
    return !isNaN(num) && num > 0;
  }, 'Weging moet positief zijn'),
});

export const netherlandsCourseListSchema = z
  .array(netherlandsCourseSchema)
  .min(1, 'Minimaal één cijfer vereist');
