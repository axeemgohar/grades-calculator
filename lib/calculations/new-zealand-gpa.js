...
export const getNZGPADescription = (gpaValue) => {
  ...
  // Note: Universities often use GPA cut-offs for Honours degrees (e.g., First Class Honours often requires ~6.5 or higher)
  if (gpa >= 8.0) return 'Outstanding Achievement (A/A+ Average)';
  if (gpa >= 6.5) return 'Excellent (First Class Honours equivalent)';
  if (gpa >= 5.0)
    return 'Very Good (Second Class Honours, First Division equivalent)';
  if (gpa >= 3.0) return 'Good (Satisfactory Pass)';
  if (gpa >= 1.0) return 'Adequate (Marginal Pass)';
